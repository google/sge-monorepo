    ...
    opt_feature = feature(
        ...
        flag_sets = [
            flag_set(
                ...
                flag_groups = [flag_group(flags = ["/O2", "/Z7"])],
            ),
            flag_set(
                actions = all_link_actions,
                flag_groups = [
                    flag_group(
                        flags = [ctx.attr.opt_mode_debug_flag, "/INCREMENTAL:NO"],
                    ),
                ],
            ),
        ],
        implies = ["frame_pointer", "generate_pdb_file"],
    )
                ...
                flag_groups = [
                    flag_group(
                        flags = [
                            ...
                            "/wd4250",
                            "/wd4996",
                            # on clang we need to force msc-version so we don't lookup the system cl.exe for it
                            # 1925 maps tp 14.25.28610 and VS 2019 Update 5
                        ] + (["-fmsc-version=1925"] if ctx.attr.compiler == "clang-cl" else []),
                    ),
                ],
            ),
        ],
                ...
                flag_groups = [
                    flag_group(
                        flag_groups = [
                            flag_group(
                                flags = [
                                    # No absolute paths in debug output.
                                    "-fdebug-compilation-dir=.",
                                    # Do not add include paths from installation.
                                    # We supply the toolchain to clang.
                                    "-nobuiltininc",
                                ],
                            ),
                        ],
        ...
        env_sets = [
            env_set(
                ...
                env_entries = [
                    ...
                    env_entry(key = "VCINSTALLDIR", value = ctx.attr.msvc_env_vcinstalldir),
                ],
            ),
        ],
...
cc_toolchain_config = rule(
    ...
    attrs = {
        ...
        "opt_mode_debug_flag": attr.string(),
