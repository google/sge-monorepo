...
# The following is based on @bazel_tools//tools/cpp/unix_cc_toolchain_config.bzl
# with minor changes to omit macos support and force sysroot.
all_compile_actions = [
...
def _impl(ctx):
    ...
    default_link_flags_feature = feature(
        ...
        flag_sets = [
            ...
            flag_set(
                ...
                flag_groups = ([
                    flag_group(
                        flags = ctx.attr.dbg_link_flags,
                    ),
                ] if ctx.attr.dbg_link_flags else []),
                with_features = [with_feature_set(features = ["dbg"])],
            ),
        ],
    )
        ...
        flag_sets = [
            flag_set(
                ...
                flag_groups = [
                    flag_group(
                        flags = [
                            "--sysroot={0}".format(ctx.attr.sysroot),
                        ],
                    ),
                ],
            ),
        ...
        flag_sets = [
            flag_set(
                ...
                flag_groups = [
                    flag_group(
                        flags = [
                            "--target={0}".format(ctx.attr.target),
                        ],
                    ),
                ],
            ),
        ...
        flag_sets = [
            flag_set(
                ...
                flag_groups = [
                    ...
                    [
                        flag_group(flags = ["-L{0}".format(a) for a in ctx.attr.library_search_directories]),
                    ] if ctx.attr.library_search_directories else []
                ),
            ),
        ],
    )
        ...
        flag_sets = [
            flag_set(
                ...
                flag_groups = [
                    ...
                    [
                        flag_group(flags = ["-I{0}".format(a) for a in ctx.attr.include_paths]),
                    ] if ctx.attr.include_paths else []
                ),
            ),
        ],
    )
    ...
    features = [
        ...
        [ target_feature ] if ctx.attr.target else []
    )
